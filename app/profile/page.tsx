...
import { INSTRUMENTS, GENRES, STATUSES, AI_TAG_CATEGORIES, USER_ROLES, getGroupsByMusicianId, getPostsByAuthorId } from '@/lib/mock-data';
...
import {
  Star, MapPin, Music, Users, Newspaper, Edit, Sparkles,
  Phone, Link2, X, Plus, Save, Check, GraduationCap, Building2, Mic2, Sliders,
} from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const handleSave = useCallback(() => {
    ...
    editName, editBio, editLocation, editPhone, editEmail,
    editStatus, editSkillLevel, editInstruments, editGenres,
    editVk, editTelegram, editYoutube, editSoundcloud, updateProfile,
  ]);
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
            ...
            <div className="flex-1">
              <div className="flex items-start justify-between gap-4 mb-3">
                <div>
                  ...
                  {currentUser.role && (() => {
                    const roleInfo = USER_ROLES.find(r => r.id === currentUser.role);
                    const RoleIcon = ROLE_ICONS[currentUser.role] ?? Music;
                    return roleInfo ? (
                      <span className="inline-flex items-center gap-1.5 mt-1.5 text-xs px-2.5 py-1 rounded-full bg-primary/10 text-primary border border-primary/20 font-medium">
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {currentUser.genres.map(genre => (
                  <Badge key={genre} className={getGenreColor(genre)}>{genre}</Badge>
                ))}
              </div>

              {currentUser.bio && (
                <p className="text-foreground text-sm leading-relaxed mb-4">{currentUser.bio}</p>
              )}
              ...
              {currentUser.socialLinks && Object.values(currentUser.socialLinks).some(Boolean) && (
                <div className="flex flex-wrap gap-2">
                  ...
                  {currentUser.socialLinks.telegram && (
                    <Badge variant="outline" className="gap-1">
                      <Link2 className="h-3 w-3" />
                      TG: @{currentUser.socialLinks.telegram}
                    </Badge>
                  )}
      ...
      <Card className="mb-6">
        <CardHeader className="pb-3">
          ...
          <CardDescription>Эти теги помогают подбирать вам подходящих партнёров</CardDescription>
        </CardHeader>
        <CardContent>
          {currentUser.aiTags.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {currentUser.aiTags.map(tag => (
                <Badge
                  key={tag.id}
                  variant="outline"
                  style={{ borderColor: getTagCategoryColor(tag.category), color: getTagCategoryColor(tag.category) }}
            ...
            <p className="text-muted-foreground text-sm">
              У вас ещё нет тегов.{' '}
              <Link href="/ai-tags" className="text-primary hover:underline">Добавьте теги</Link>,
          ...
          <CardContent className="space-y-3 text-sm">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="font-medium">{currentUser.teacherProfile.experience} лет</p>
              </div>
              <div>
                <p className="text-muted-foreground">Стоимость урока</p>
                <p className="font-medium">{currentUser.teacherProfile.pricePerHour} ₽/час</p>
              </div>
            </div>
            {currentUser.teacherProfile.education && (
              <div>
                <p className="text-muted-foreground">Образование</p>
                <p className="font-medium">{currentUser.teacherProfile.education}</p>
              </div>
            )}
            {currentUser.teacherProfile.subjects?.length > 0 && (
              <div>
                <p className="text-muted-foreground mb-1.5">Дисциплины</p>
                <div className="flex flex-wrap gap-1.5">{currentUser.teacherProfile.subjects.map(s => <Badge key={s} variant="secondary">{s}</Badge>)}</div>
              </div>
            )}
            {currentUser.teacherProfile.lessonFormats?.length > 0 && (
              <div>
                <p className="text-muted-foreground mb-1.5">Формат занятий</p>
                <div className="flex flex-wrap gap-1.5">{currentUser.teacherProfile.lessonFormats.map(f => <Badge key={f} variant="outline">{f}</Badge>)}</div>
              </div>
            )}
            {currentUser.teacherProfile.ageGroups?.length > 0 && (
              <div>
                <p className="text-muted-foreground mb-1.5">Возрастные группы</p>
                <div className="flex flex-wrap gap-1.5">{currentUser.teacherProfile.ageGroups.map(a => <Badge key={a} variant="outline">{a}</Badge>)}</div>
              </div>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-3 text-sm">
            {currentUser.producerProfile.specialization?.length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">{currentUser.producerProfile.specialization.map(s => <Badge key={s} variant="secondary">{s}</Badge>)}</div>
              </div>
            )}
            {currentUser.producerProfile.services?.length > 0 && (
              <div>
                <p className="text-muted-foreground mb-1.5">Услуги</p>
                <div className="flex flex-wrap gap-1.5">{currentUser.producerProfile.services.map(s => <Badge key={s} variant="outline">{s}</Badge>)}</div>
              </div>
            )}
            {currentUser.producerProfile.artistsWorkedWith && (
              <div>
                <p className="text-muted-foreground">Опыт работы</p>
                <p className="font-medium">{currentUser.producerProfile.artistsWorkedWith}</p>
              </div>
            )}
            {currentUser.producerProfile.labelAffiliation && (
              <div>
                <p className="text-muted-foreground">Лейбл / агентство</p>
                <p className="font-medium">{currentUser.producerProfile.labelAffiliation}</p>
              </div>
            )}
          </CardContent>
        </Card>
      ...
      {currentUser.role === 'sound_engineer' && currentUser.soundEngineerProfile && (
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base">
          ...
          <CardContent className="space-y-3 text-sm">
            {currentUser.soundEngineerProfile.specialization?.length > 0 && (
              <div>
                <p className="text-muted-foreground mb-1.5">Специализация</p>
                <div className="flex flex-wrap gap-1.5">{currentUser.soundEngineerProfile.specialization.map(s => <Badge key={s} variant="secondary">{s}</Badge>)}</div>
              </div>
            )}
            {currentUser.soundEngineerProfile.software?.length > 0 && (
              <div>
                <p className="text-muted-foreground mb-1.5">ПО</p>
                <div className="flex flex-wrap gap-1.5">{currentUser.soundEngineerProfile.software.map(s => <Badge key={s} variant="outline">{s}</Badge>)}</div>
              </div>
            )}
            {currentUser.soundEngineerProfile.hardwareSummary && (
              <div>
                <p className="text-muted-foreground">Оборудование</p>
                <p className="font-medium">{currentUser.soundEngineerProfile.hardwareSummary}</p>
              </div>
            )}
            {currentUser.soundEngineerProfile.studioAffiliation && (
              <div>
                <p className="text-muted-foreground">Студия / место работы</p>
                <p className="font-medium">{currentUser.soundEngineerProfile.studioAffiliation}</p>
              </div>
            )}
          </CardContent>
        </Card>
          ...
          {userGroups.length > 0 ? (
            ...
            <Card>
              <CardContent className="py-8 text-center">
                ...
                <Button asChild><Link href="/groups">Найти группу</Link></Button>
              </CardContent>
            </Card>
          )}
        </TabsContent>
          ...
          {userPosts.length > 0 ? (
            ...
            <Card>
              <CardContent className="py-8 text-center">
                ...
                <Button asChild><Link href="/feed">Перейти в ленту</Link></Button>
              </CardContent>
            </Card>
          )}
        </TabsContent>
          ...
          <div className="grid gap-5 py-2">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="grid gap-2">
                ...
                <Input id="edit-name" value={editName} onChange={e => setEditName(e.target.value)} placeholder="Ваше имя" />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="edit-location">Город</Label>
                <Input id="edit-location" value={editLocation} onChange={e => setEditLocation(e.target.value)} placeholder="Москва" />
              </div>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="grid gap-2">
                ...
                <Input id="edit-phone" value={editPhone} onChange={e => setEditPhone(e.target.value)} placeholder="+7 (999) 000-00-00" />
              </div>
            </div>
              ...
              <div className="grid gap-2">
                ...
                <div className="flex items-center gap-2 h-10">
                  {[1, 2, 3, 4, 5].map(level => (
                    ...
                    >
                      <Star className={`h-6 w-6 transition-colors ${level <= editSkillLevel ? 'fill-warning text-warning' : 'text-muted-foreground'}`} />
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div className="grid gap-2">
              ...
              <div className="flex flex-wrap gap-2">
                {INSTRUMENTS.map(instr => (
                  ...
                  >
                    {editInstruments.includes(instr) && <Check className="h-3 w-3" />}
                    {instr}
                  </button>
                ))}
              </div>
            ...
            <div className="grid gap-2">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="grid gap-1.5">
                  <Label htmlFor="edit-vk" className="text-xs text-muted-foreground">VK (username)</Label>
                  <Input id="edit-vk" value={editVk} onChange={e => setEditVk(e.target.value)} placeholder="my_vk_page" />
                </div>
                <div className="grid gap-1.5">
                  <Label htmlFor="edit-tg" className="text-xs text-muted-foreground">Telegram (username)</Label>
                  <Input id="edit-tg" value={editTelegram} onChange={e => setEditTelegram(e.target.value)} placeholder="mytelegram" />
                </div>
                <div className="grid gap-1.5">
                  <Label htmlFor="edit-yt" className="text-xs text-muted-foreground">YouTube (channel)</Label>
                  <Input id="edit-yt" value={editYoutube} onChange={e => setEditYoutube(e.target.value)} placeholder="mychannel" />
                </div>
                <div className="grid gap-1.5">
                  <Label htmlFor="edit-sc" className="text-xs text-muted-foreground">SoundCloud (username)</Label>
                  <Input id="edit-sc" value={editSoundcloud} onChange={e => setEditSoundcloud(e.target.value)} placeholder="mysoundcloud" />
                </div>
              </div>
            </div>
          </div>
