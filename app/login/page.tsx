...
import {
  INSTRUMENTS, GENRES, USER_ROLES, VENUE_TYPES,
  type UserRole, type Musician,
  type TeacherProfile, type ProducerProfile,
  type SoundEngineerProfile, type JournalistProfile,
} from '@/lib/mock-data';
...
import {
  Music, GraduationCap, Building2, Mic2, Sliders, Newspaper,
  ChevronRight, ChevronLeft, X, Plus, Check, Users,
} from 'lucide-react';
...
const emptyBase: BaseForm = {
  ...
  instruments: [], genres: [], skillLevel: 3,
};

// --- Форма деталей по ролям ---

function MusicianDetails({ base, setBase }: { base: BaseForm; setBase: (b: BaseForm) => void }) {
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        <StarRating value={base.skillLevel} onChange={v => setBase({ ...base, skillLevel: v })} />
        <p className="text-xs text-muted-foreground">
          {['', 'Начинающий', 'Любитель', 'Средний уровень', 'Продвинутый', 'Профессионал'][base.skillLevel]}
        </p>
      </div>
    </div>
  );
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            onChange={e => setData({ ...data, pricePerHour: Number(e.target.value) })}
            placeholder="1500"
          />
        </div>
      </div>
...
function VenueDetails({
  ...
  data: { venueName: string; venueType: string; address: string; pricePerHour: string; equipment: string[]; description: string };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        <Select value={data.venueType} onValueChange={v => setData({ ...data, venueType: v })}>
          <SelectTrigger>
...
function ProducerDetails({
  ...
  const specs = ['Продюсирование', 'Промоутинг', 'Буккинг', 'A&R', 'Менеджмент', 'Event-организация'];
  const svcs = ['Продакшн', 'Буккинг', 'PR-поддержка', 'Организация туров', 'Студийное время', 'Мерч'];
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        <Textarea
          ...
          onChange={e => setData({ ...data, artistsWorkedWith: e.target.value })}
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          onChange={e => setData({ ...data, labelAffiliation: e.target.value })}
...
function SoundEngineerDetails({
  ...
  const specs = ['Запись', 'Сведение', 'Мастеринг', 'Live-звук', 'Саунд-дизайн', 'Подкасты'];
  const sw = ['Pro Tools', 'Logic Pro', 'Ableton Live', 'Cubase', 'FL Studio', 'Reaper', 'Waves', 'iZotope'];
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        <Textarea
          ...
          onChange={e => setData({ ...data, hardwareSummary: e.target.value })}
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          onChange={e => setData({ ...data, studioAffiliation: e.target.value })}
...
function JournalistDetails({
  ...
  const specs = ['Рецензии', 'Интервью', 'Репортажи', 'Live-фото', 'Подкасты', 'Редактура'];
...
export default function LoginPage() {
  ...
  const [venueData, setVenueData] = useState({
    venueName: '', venueType: '', address: '', pricePerHour: '', equipment: [] as string[], description: '',
  });
  ...
  const [journalistData, setJournalistData] = useState<Partial<JournalistProfile>>({});
  ...
  const handleRegisterSubmit = () => {
    ...
    const userData: Omit<Musician, 'id' | 'aiTags' | 'status' | 'avatar'> = {
      ...
      bio: base.bio,
      ...(selectedRole === 'teacher' && { teacherProfile: teacherData as TeacherProfile }),
      ...(selectedRole === 'producer' && { producerProfile: producerData as ProducerProfile }),
      ...(selectedRole === 'sound_engineer' && { soundEngineerProfile: soundData as SoundEngineerProfile }),
      ...(selectedRole === 'journalist' && { journalistProfile: journalistData as JournalistProfile }),
    };
    ...
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-xl">
        ...
        <div className="text-center mb-6">
          ...
          <h1 className="text-xl font-bold text-foreground">Регистрация в УМПСМ</h1>
        </div>
            ...
            {progressSteps.map((s, i) => (
              <div key={s} className="flex items-center gap-2">
                ...
                <span className={cn(
                  ...
                  currentStepIndex === i ? 'text-primary font-medium' : 'text-muted-foreground'
                )}>
                  {stepLabels[s]}
                </span>
                {i < progressSteps.length - 1 && (
                  <div className={cn('w-8 h-0.5', currentStepIndex > i ? 'bg-primary' : 'bg-border')} />
                )}
              </div>
            ))}
          </div>
        ...
        {step === 'role' && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Выберите вашу роль — от этого зависит профиль и возможности</CardDescription>
            </CardHeader>
        ...
        {step === 'base' && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Заполните базовые данные вашего профиля</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2 space-y-2">
                    <Label>Имя и фамилия <span className="text-destructive">*</span></Label>
                    <Input
                      value={base.name}
        ...
        {step === 'role_details' && selectedRole && (
          <Card>
            <CardHeader>
              <CardTitle>
                {USER_ROLES.find(r => r.id === selectedRole)?.label} — детали профиля
              </CardTitle>
              <CardDescription>
              ...
              <div className="space-y-4">
                {selectedRole === 'musician' && (
                  <p className="text-sm text-muted-foreground bg-muted rounded-lg p-3">
                    Вы уже заполнили музыкальные данные на предыдущем шаге. Можно сразу завершить регистрацию.
                  </p>
                )}
                ...
                {selectedRole === 'producer' && (
                  <ProducerDetails data={producerData} setData={setProducerData} />
                )}
                {selectedRole === 'sound_engineer' && (
                  <SoundEngineerDetails data={soundData} setData={setSoundData} />
                )}
                {selectedRole === 'journalist' && (
                  <JournalistDetails data={journalistData} setData={setJournalistData} />
                )}

                <Separator />

                <div className="flex items-center justify-between pt-1">
                  <Button variant="ghost" size="sm" onClick={() => setStep('base')}>
                    <ChevronLeft className="h-4 w-4 mr-1" />
                    Назад
                  </Button>
                  <Button onClick={handleRegisterSubmit} disabled={!base.name || !base.email}>
