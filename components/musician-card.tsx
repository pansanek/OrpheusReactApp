...
export function MusicianCard({
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "recording":
        return "bg-[var(--status-recording)] animate-pulse";
      default:
    ...
    <>
      <Card className="hover:shadow-lg transition-all duration-200 hover:-translate-y-0.5">
        <CardHeader className="pb-3">
          <div className="flex items-start gap-4">
            <div className="relative">
              ...
              <span
                className={`absolute bottom-0 right-0 w-4 h-4 ${getStatusColor(musician.status)} border-2 border-card rounded-full`}
              />
            </div>
        ...
        <CardContent className="pt-0">
          ...
          {showActions && currentUser && !isOwnCard && (
